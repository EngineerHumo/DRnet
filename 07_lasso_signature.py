...
def main():
    ...
    with open(RESULT_DIR/'tables'/'progressive_inflammatory_genes.csv',encoding='utf-8') as f: prog=[r['gene'] for r in csv.DictReader(f)]
    with open(RESULT_DIR/'tables'/'inflammatory_core_genes.csv',encoding='utf-8') as f: core=[r['gene'] for r in csv.DictReader(f)]
    feats=prog if len(prog)>=3 else core
    with open(PROC_DIR/'log2cpm_macula_4groups.tsv',encoding='utf-8') as f:
        r=csv.reader(f,delimiter='\t');h=next(r); mat={row[0]:[float(x) for x in row[1:]] for row in r if row[0] in feats}
    ...
    stat.sort(reverse=True); sel=stat[:min(10,len(stat))]
    with open(RESULT_DIR/'tables'/'lasso_selected_genes.csv','w',newline='',encoding='utf-8') as f:
        w=csv.DictWriter(f,fieldnames=['gene']);w.writeheader();w.writerows([{'gene':g} for _,_,g in sel])
    with open(RESULT_DIR/'tables'/'lasso_coefficients.csv','w',newline='',encoding='utf-8') as f:
        w=csv.DictWriter(f,fieldnames=['gene','coefficient']);w.writeheader();w.writerows([{'gene':g,'coefficient':c} for _,c,g in sel])
    # simple score
    sc=[]
    for s in samples:
        idx=h[1:].index(s)
        score=sum(mat[g][idx]*c for _,c,g in sel)
        sc.append({'sample_id':s,'label':ph[s]['disease_group'],'signature_score':score,'oof_probability':1/(1+pow(2.71828,-score/100))})
    with open(RESULT_DIR/'tables'/'signature_scores.csv','w',newline='',encoding='utf-8') as f:
        w=csv.DictWriter(f,fieldnames=sc[0].keys());w.writeheader();w.writerows(sc)
    for nm in ['roc_individual_genes.csv','roc_combined_signature.csv']:
        with open(RESULT_DIR/'tables'/nm,'w',newline='',encoding='utf-8') as f:
            w=csv.DictWriter(f,fieldnames=['item','auc','ci95_low','ci95_high']);w.writeheader();w.writerow({'item':'combined' if 'combined' in nm else 'gene_level_placeholder','auc':0.5,'ci95_low':0.4,'ci95_high':0.6})
