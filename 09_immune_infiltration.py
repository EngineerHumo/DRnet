...
from pipeline_utils import (
    ensure_dirs, read_gmt, ssgsea_score, mann_whitney_u, spearman, bh_adjust,
...
def main():
    ...
    mat, _ = matrix_ensembl_to_symbol(mat_ensembl, mapping)
    ...
    for c in cells:
        ...
        comp.append({'cell_type': c, 'pvalue': p})

        rr = [r for r in scores if r['cell_type'] == c]
        rho, p2 = spearman([int(a['severity_code']) for a in rr], [a['score'] for a in rr])
        trend.append({'cell_type': c, 'rho': rho, 'pvalue': p2})

    for arr in [comp, trend]:
        adj = bh_adjust([r['pvalue'] for r in arr])
        for r, a in zip(arr, adj):
            r['padj'] = a
    ...
    for g in genes:
        ...
        for c in sig:
            ...
            cor.append({'gene_symbol': g, 'cell_type': c, 'rho': rho, 'pvalue': p})

    with open(RESULT_DIR / 'tables' / 'gene_immune_correlations.csv', 'w', newline='', encoding='utf-8') as f:
        fields = cor[0].keys() if cor else ['gene_symbol', 'cell_type', 'rho', 'pvalue']
