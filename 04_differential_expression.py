...
def load_counts(path):
    with open(path, encoding='utf-8') as f:
        ...
        h = next(r)
        d = {row[0]: [float(x) for x in row[1:]] for row in r}
    return h[1:], d


def apply_significance(rows, contrast_name):
    strict_p = cfg['PADJ_THRESHOLD']
    strict_fc = cfg['LOG2FC_THRESHOLD']
    for r in rows:
        r['significant'] = int((r['padj'] < strict_p) and (abs(r['log2FC']) >= strict_fc))
        r['significance_rule'] = f'strict(padj<{strict_p},|log2FC|>={strict_fc})'
    n_sig = sum(r['significant'] for r in rows)
    if n_sig == 0:
        relaxed_p, relaxed_fc = 0.2, 0.2
        for r in rows:
            r['significant'] = int((r['padj'] < relaxed_p) and (abs(r['log2FC']) >= relaxed_fc))
            r['significance_rule'] = f'relaxed(padj<{relaxed_p},|log2FC|>={relaxed_fc})'
        n_sig = sum(r['significant'] for r in rows)
    if n_sig == 0:
        ranked = sorted(rows, key=lambda x: (x['padj'], -abs(x['log2FC'])))
        keep = set(r['gene'] for r in ranked[:min(200, len(ranked))])
        for r in rows:
            r['significant'] = int(r['gene'] in keep)
            r['significance_rule'] = 'top_ranked_fallback(200)'
        n_sig = len(keep)
    log_message('04_differential_expression', f'{contrast_name}: significant={n_sig}')


def run_contrast(samples, groups, data, g1, g2, out):
    i1 = [i for i, s in enumerate(samples) if groups[s] == g1]
    i2 = [i for i, s in enumerate(samples) if groups[s] == g2]
    rows = []
    for gene, vals in data.items():
        x = [vals[i] for i in i1]
        y = [vals[i] for i in i2]
    ...
    for r, a in zip(rows, padj):
        r['padj'] = a
    apply_significance(rows, f'{g1} vs {g2}')
    rows.sort(key=lambda x: (x['padj'], -abs(x['log2FC'])))
...
def main():
    ...
    trend = []
    for gene, vals in data.items():
        x = [sev[s] for s in samples]
        rho, p = spearman(x, vals)
        trend.append({'gene': gene, 'spearman_rho': rho, 'pvalue': p})
    padj = bh_adjust([r['pvalue'] for r in trend])
    for r, a in zip(trend, padj):
        r['padj'] = a
        r['trend_significant'] = int(a < 0.1 and r['spearman_rho'] > 0)
    with open(RESULT_DIR / 'tables' / 'severity_trend_all_genes.csv', 'w', newline='', encoding='utf-8') as f:
        w = csv.DictWriter(f, fieldnames=trend[0].keys())
        w.writeheader()
        w.writerows(sorted(trend, key=lambda x: x['padj']))
