...
def main():
    ...
    for g in genes:
        ...
        for p,gl in hall.items():
            ...
            rows.append({'gene':g,'pathway':p,'overlap':overlap,'spearman_rank_score':overlap})
    ...
    with open(RESULT_DIR/'tables'/'gsea_summary_matrix.csv','w',newline='',encoding='utf-8') as f:
        w=csv.DictWriter(f,fieldnames=summ[0].keys() if summ else ['gene','pathway','overlap','spearman_rank_score']);w.writeheader();w.writerows(summ)
    rec={}
    for r in summ: rec[r['pathway']]=rec.get(r['pathway'],0)+r['overlap']
    with open(RESULT_DIR/'tables'/'pathway_recurrence_summary.csv','w',newline='',encoding='utf-8') as f:
        w=csv.DictWriter(f,fieldnames=['pathway','hit_genes']);w.writeheader();w.writerows([{'pathway':k,'hit_genes':v} for k,v in sorted(rec.items(), key=lambda x:-x[1])])
    with open(RESULT_DIR/'tables'/'ipa_input_selected_genes.csv','w',newline='',encoding='utf-8') as f:
        w=csv.DictWriter(f,fieldnames=['gene']);w.writeheader();w.writerows([{'gene':g} for g in genes])
